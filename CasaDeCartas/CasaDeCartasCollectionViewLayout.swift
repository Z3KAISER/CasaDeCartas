//
//  CasaDeCartasCollectionViewLayout.swift
//  Pods
//
...
public class CasaDeCartasCollectionViewLayout: UICollectionViewLayout, UIGestureRecognizerDelegate {
    ...
    var delegate: CasaDeCartasViewControllerDelegate?
    ...
    var gesturesEnabled: Bool = false {
        didSet {
            if (gesturesEnabled) {
                let recognizer = UIPanGestureRecognizer(target: self, action: #selector(CasaDeCartasCollectionViewLayout.handlePan(_:)))
                collectionView?.addGestureRecognizer(recognizer)
                panGestureRecognizer = recognizer
                panGestureRecognizer!.delegate = self
                
                swipeRecognizerDown = UISwipeGestureRecognizer(target: self, action:  #selector(CasaDeCartasCollectionViewLayout.handleSwipe(_:)))
                ...
                swipeRecognizerUp = UISwipeGestureRecognizer(target: self, action:  #selector(CasaDeCartasCollectionViewLayout.handleSwipe(_:)))
    ...
    //
    private var nopeStack = [Int]()
    
    //
    private var yepStack = [Int]()
    ...
    public override func layoutAttributesForItemAtIndexPath(indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        } else if (nopeStack.contains(indexPath.item)) {
            result = layoutAttributesForNopeStackItemWithInitialAttributes(result)
            //we have to hide invisible cards in bottom stask due perfomance issues due shadows
            result.hidden = true
        } else if (yepStack.contains(indexPath.item)) {
            result = layoutAttributesForYepStackItemWithInitialAttributes(result)
            //we have to hide invisible cards in bottom stask due perfomance issues due shadows
            result.hidden = true
        }
    ...
    private func layoutAttributesForNopeStackItemWithInitialAttributes(attributes: UICollectionViewLayoutAttributes) -> UICollectionViewLayoutAttributes {
        //        let yPosition = collectionView!.frame.size.height - bottomStackCardHeight
        //        let yPosition = collectionView!.frame.size.height / 3
        let yPosition = 0
        //        let xPosition = (collectionView!.frame.size.width - cardSize.width)/2
        let xPosition = -collectionView!.frame.size.width * 2
    ...
    private func layoutAttributesForYepStackItemWithInitialAttributes(attributes: UICollectionViewLayoutAttributes) -> UICollectionViewLayoutAttributes {
        //        let yPosition = collectionView!.frame.size.height - bottomStackCardHeight
        //        let yPosition = collectionView!.frame.size.height / 3
        let yPosition = 0
        //        let xPosition = (collectionView!.frame.size.width - cardSize.width)/2
    ...
    private func finishedDragging(cell: UICollectionViewCell) {
        let signedDeltaX = cell.center.x - initialCellCenter!.x
        ...
        if shouldSnapBack {
            ...
            //
            if (signedDeltaX > 0) {
                yepStack.append(index)
            } else {
                nopeStack.append(index)
            }
